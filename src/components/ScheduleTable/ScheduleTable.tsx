...
const ScheduleTable: React.FC<ScheduleMeetingDialogProps> = ({
  ...
  const handleDateChange = (value: DateObject) => {
    ...
    if (value) {
      ...
      handleGetTimeSlots(value.day, date.month() + 1, date.year());
  ...
  const handleGetTimeSlots = (day: number, month: number, year: number) => {
    ...
    const endpoint = `https://localhost:7256/api/Reservation/TimeSlots?TableId=${tableLabel}&Day=${day}&Month=${month}&Year=${year}`;
  ...
  const handleConfirmReservation = () => {
    ...
    if (chosenDay && chosenMonth && chosenYear && chosenTimeSlotId) {
      const endpoint = "https://localhost:7256/api/Reservation";
